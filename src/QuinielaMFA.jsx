...
function StandingsView({ matches, predictions, calcPoints, user }) {
  ...
  React.useEffect(() => {
    const load = async () => {
      setLoading(true);
      const { data: usuarios } = await supabase.from("usuarios").select("email, nombre, primer_apellido, nombre_comercial, provincia, bloqueado, bono_campeon, bono_goleador, bono_mvp");
      const { data: resultados } = await supabase.from("resultados").select("*").eq("published", true);
      const { data: knockoutResultados } = await supabase.from("knockout_resultados").select("*").eq("published", true);
